...
PROJ="$ROOT/src/Milestones/Milestones.csproj"
DLL="$ROOT/src/Milestones/bin/Release/net472/Milestones.dll"
...
ASM_VERSION=$(grep -oP '(?<=PluginVersion = ")[^"]+' "$ROOT/src/Milestones/Plugin.cs")
...
echo "==> staging"
rm -rf "$STAGE"
mkdir -p "$STAGE/plugins/Milestones"
cp "$DLL"                        "$STAGE/plugins/Milestones/"
...
OUT="$ROOT/dist/Milestones-$VERSION.zip"
rm -f "$OUT"
( cd "$STAGE" && zip -qr "$OUT" . )
